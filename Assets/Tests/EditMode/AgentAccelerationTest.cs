...
public class AgentTests : AgentTestBase {
  ...
  public override void Setup() {
    ...
    var threatConfig = new DynamicAgentConfig {
      ...
      standard_deviation =
          new StandardDeviation { position = Vector3.zero, velocity = Vector3.zero },
      dynamic_config =
    ...
    var interceptorConfig = new DynamicAgentConfig {
      ...
      standard_deviation =
          new StandardDeviation { position = Vector3.zero, velocity = Vector3.zero },
      dynamic_config =
          new DynamicConfig { sensor_config =
                                  new SensorConfig { type = SensorType.IDEAL, frequency = 100 },
  ...
  public override void Teardown() {
    base.Teardown();
    if (testThreat != null)
      GameObject.DestroyImmediate(testThreat.gameObject);
    if (testInterceptor != null)
      GameObject.DestroyImmediate(testInterceptor.gameObject);
  }
    ...
    var threatConfig = new DynamicAgentConfig {
      ...
      dynamic_config =
          new DynamicConfig {
            ...
            flight_config = new FlightConfig { evasionEnabled = false, augmentedPnEnabled = false }
          }
    };
    ...
    testThreat.dynamicAgentConfig = new DynamicAgentConfig {
      ...
      dynamic_config =
          new DynamicConfig { launch_config = new LaunchConfig { launch_time = 0 },
                              sensor_config =
                                  new SensorConfig { type = SensorType.IDEAL, frequency = 100 },
                              flight_config = new FlightConfig { evasionEnabled = false,
  ...
  private bool IsAccelerationPhysicallyReasonable(Vector3 acceleration, Agent agent) {
    if (!float.IsFinite(acceleration.x) || !float.IsFinite(acceleration.y) ||
        !float.IsFinite(acceleration.z)) {
      return false;
    }
