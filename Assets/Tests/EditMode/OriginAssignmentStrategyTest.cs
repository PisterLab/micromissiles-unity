...
public class OriginAssignmentStrategyTest : TestBase {
    ...
    public void SetUp() {
        ...
        _testOrigins = new List<InterceptorOriginConfig> {
            ...
            new InterceptorOriginConfig {
                id = "CVN-68-Nimitz",
                initial_position = new Vector3(0, 0, 5000),
                velocity = new Vector3(0, 0, -12), // 12 m/s forward
                max_interceptors = 24,
                interceptor_types = new List<string> { "hydra70.json", "micromissile.json", "f18_missile.json" }
            },
            // Destroyer - Medium capacity, specialized interceptors
            new InterceptorOriginConfig {
                id = "DDG-51-Burke",
                initial_position = new Vector3(-2000, 0, 4000),
                velocity = new Vector3(0, 0, -12), // Same formation speed
                max_interceptors = 8,
                interceptor_types = new List<string> { "sm2.json", "sm6.json" }
            },
            // Cruiser - Medium capacity, long-range interceptors
            new InterceptorOriginConfig {
                id = "CG-47-Ticonderoga",
                initial_position = new Vector3(2000, 0, 4000),
                velocity = new Vector3(0, 0, -12), // Same formation speed
                max_interceptors = 12,
            ...
            new InterceptorOriginConfig {
                id = "Aegis-Ashore-1",
                initial_position = new Vector3(10000, 100, 0),
                velocity = Vector3.zero, // Static installation
                max_interceptors = 48,
                interceptor_types = new List<string> { "sm3.json", "thaad.json" }
            }
        };
    ...
    public void TestClosestOriginStrategy_SingleTarget() {
        ...
        InterceptorOriginConfig selectedOrigin = _originManager.SelectOrigin(
            threatPosition, 
            requestedInterceptorType, 
            OriginAssignmentStrategy.CLOSEST,
            0f // Current time
        );
    ...
    public void TestClosestOriginStrategy_NoCompatibleOrigin() {
        ...
        InterceptorOriginConfig selectedOrigin = _originManager.SelectOrigin(
            threatPosition, 
            unsupportedInterceptorType, 
            OriginAssignmentStrategy.CLOSEST,
            0f
        );

        Assert.IsNull(selectedOrigin, "Should return null when no origin supports the interceptor type");
    }
    ...
    public void TestCapabilityBasedStrategy() {
        ...
        selectedOrigin = _originManager.SelectOrigin(
            threatPosition, 
            carrierInterceptor, 
            OriginAssignmentStrategy.CAPABILITY_BASED,
            0f
    ...
    public void TestManualStrategy() {
        ...
        InterceptorOriginConfig selectedOrigin = _originManager.SelectOrigin(
            threatPosition, 
            interceptorType, 
            OriginAssignmentStrategy.MANUAL,
            0f,
            manuallySpecifiedOriginId
        );
    ...
    public void TestOriginSelection_PriorityOrder() {
        ...
        for (int i = 0; i < 10; i++) {
            InterceptorOriginConfig selectedOrigin = _originManager.SelectOrigin(
                centralThreatPosition, 
                commonInterceptorType, 
                OriginAssignmentStrategy.CLOSEST,
                0f
            );
    ...
    public void TestOriginManager_GetAvailableOrigins() {
        ...
        List<InterceptorOriginConfig> availableOrigins = _originManager.GetAvailableOrigins(interceptorType);
        ...
        Assert.AreEqual(0, availableOrigins.Count, "No origins should be available when capacity is exhausted");
    }
}
